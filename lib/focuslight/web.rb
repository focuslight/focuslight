...
class Focuslight::Web < Sinatra::Base
  ...
  get '/xport/:service_name/:section_name/:graph_name', :graph => :simple do
    req_params = validate(params, graph_rendering_request_spec)
    MultiJson.dump(rrd().export(request.stash[:graph], req_params.hash), allow_nan: true).gsub(/NaN/, "null")
  end
  ...
  get '/xport/:complex' do
    ...
    MultiJson.dump(rrd().export(data, req_params.hash), allow_nan: true).gsub(/NaN/, "null")
  end
